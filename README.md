# Orysys
#Assumptions -->
  1. Assumed the Login is hardcoded and the login user creation is not implemented and password validations (length, text, numeric, etc..) also not considered here.
  2. Assumed Interest Rate can be edited when registering loan application even the interest rate is autofilled when selecting the Loan Type
  3. Assumed multiple time the registered applications status can be updated
  4. Loan Application update can be perform only for status update
